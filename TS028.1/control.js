  ...
  function checkCardSellingFast(card) {
    ...
    if (chosenTimes.length) {
      if (!TRACKING_FLAGS.urgencyPageFired) {
        ...
        pushEvent(
          "TS028.1 | (Control Original) | Pages with High Availability Slots",
          "TS028.1ECOQ",
        );
      }
    }
  ...
  function bindMobileDropdown(card) {
    ...
    button.addEventListener("click", function () {
      if (window.innerWidth <= 1024) {
        ...
        pushEvent(
          "TS028.1 | (Control Original) | Time slot drop down opened",
          "TS028.1ECOG",
        );
      }
    });
    ...
    card.addEventListener("click", function (e) {
      ...
      if (bookLink && card.contains(bookLink)) {
        if (getCardSellingFastTimes(card).length) {
          ...
          pushEvent(
            "TS028.1 | (Control Original) | High Availability slot Book Now Click",
            "TS028.1ECOI",
          );
        } else {
          // 6 Non High Availability BOOK NOW Click
          pushEvent(
            "TS028.1 | (Control Original) | Non High Availability BOOK NOW Click",
            "TS028.1ECOK",
          );
        }
        ...
        pushEvent(
          "TS028.1 | (Control Original) | Overall BOOK NOW Click",
          "TS028.1ECOM",
        );
        return;
      }
      ...
      if (
        ...
        if (isTimeText(btnText)) {
          ...
          pushEvent(
            "TS028.1 | (Control Original) | Overall Slot Click",
            "TS028.1ECOL",
          );
          if (chosenTimes.indexOf(btnText) > -1) {
            // 3 High Availability slot Click
            pushEvent(
              "TS028.1 | (Control Original) | High Availability slot Click",
              "TS028.1ECOH",
            );
          } else {
            // 5 Non High Availability slot Click
            pushEvent(
              "TS028.1 | (Control Original) | Non High Availability slot Click",
              "TS028.1ECOJ",
            );
          }
          return;
