...
// 1 Subscription is present on PDP
function checkSubscriptionPresence() {
  let attempts = 0;
...
function trackEvents() {
  if (!document.querySelector("body").classList.contains("LT147")) {
    document.querySelector("body").classList.add("LT147");
    document.addEventListener("click", (e) => {
      ...
      if (
        e.target.closest(
          ".info-panel-one-time-single-bottle .add-to-cart-btn",
        ) ||
      ...
      // 5. ATB from Subscribe & Save panel - fires only on successful ATB
      if (
        e.target.closest(
          ".info-panel-subscribe-single-bottle .add-to-cart-btn",
        ) ||
