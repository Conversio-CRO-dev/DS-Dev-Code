console.log("====================================================>");
console.log("David Adam Silva | LT153 - LT151/columns product list");
console.log("====================================================>");
...
async function getNextData() {
  // Seed from __NEXT_DATA__ on first load
  ...
  if (nextDataScript) {
    ...
    if (productData) {
      ...
      // console.log("Seeded latestHits from __NEXT_DATA__:", latestHits.length, "products");
    }
  }
}
...
async function analyzeLaithwaitesPricing() {
  ...
  // find all prod items in the main PLP grid
  const allLinks = await allElementsReady(
    '.ais-Hits-list .ais-Hits-item a[href*="/product/"]',
  );
  let promoBannerLoggedThisPageView = false;

  const mainProducts = Array.from(allLinks).filter((link) => {
    // get item code
    const url = new URL(link.href);
    const pathParts = url.pathname.split("/");

    // Must have at least /product/something
    ...
    // 1. /product/item-code
    // 2. /product/product-name/item-code
    let itemCode;
    if (pathParts.length === 3) {
      itemCode = pathParts[2];
    } else if (pathParts.length >= 4) {
      itemCode = pathParts[3];
    } else {
      return false;
    }

    return itemCode && itemCode.match(/^[A-Za-z0-9]{4,}$/);
  });

  // Group by itemCode (some products appear multiple times)
  const productsByItemCode = new Map();
  mainProducts.forEach((link) => {
    const itemCode = link.href.split("/").pop().split("?")[0];
    if (!productsByItemCode.has(itemCode)) {
      productsByItemCode.set(itemCode, link);
    }
  });

  productsByItemCode.forEach(async (linkElement, itemCode) => {
    // Find matching JSON data
    const jsonProduct = productsJSON.find((p) => p.itemCode === itemCode);

    // Skip products not in JSON
    if (!jsonProduct) {
      return;
    }

    // when prod found on json file apply the pricing panel on the card
    if (jsonProduct) {
      let prodInfoObj = jsonProduct;
      let itemCont = linkElement.closest(".ais-Hits-item");
      if (!itemCont) return;
      ...
      // add subType attribute to item, to then later use in add to bag event
      ...
      // add product type attribute to later use in ATB event
      let porductType = prodInfoObj.productType;
      if (porductType) {
        itemCont.setAttribute("product-type", porductType);
      }

      if (prodInfoObj.offers.length === 0 && sku.numberOfBottles) {
        // RRP and sale price the same - no offer item
        let priceText;

        if (sku.numberOfBottles === 1) {
          priceText = "per bottle";
        } else {
          priceText = "per " + sku.numberOfBottles + " case";
        }
        ...
        sku.buyersRRP !== sku.salePrice &&
        sku.numberOfBottles &&
        !prodInfoObj.promotionalData
      ) {
        // add SIMPLE OFFER PRICE (red backgrounds and savings shown)
        let priceText =
          sku.numberOfBottles === 1
            ? "per bottle"
            : "per " + sku.numberOfBottles + " bottle case";
        let savingsDiff = sku.buyersRRP - sku.salePrice;
        ...
        // PROMO PRICING - offer + promo(red + purple designs)
        let tieredDisc = prodInfoObj.promotionalData[0].tierUIRules[0].quantity;
        ...
        if (tieredDisc > 1) {
          ...
          // add promo pricing
          let promoPrice = prodInfoObj.promotionalData[0].tierUIRules[0].amount;
          let savingsDiffPromo = sku.buyersRRP - promoPrice;

          if (promoPrice) {
            let promoTextToBuy =
              sku.numberOfBottles === 1 ? "bottles" : "cases";
            ...
            promoPricing.innerHTML =
              ...
              '<div class="savings-text">Save £</div>' +
              '<div class="savings-amount">' +
              savingsDiffPromo.toFixed(2) +
              "</div>" +
              "</div>" +
              "</div>" +
              '<div class="promo-rule">When you buy <span>' +
              tieredDisc +
              " " +
              promoTextToBuy +
              "<span></div>";
        ...
        // Fallback for cards like M20043.
        // Do not touch the existing logic above. Only patch cards that fell through.
        const hasExistingCustomPricing = customCont.querySelector(
          ".offer-pricing, .promo-pricing, .no-offer-pricing",
        );
        const hasVisibleSaving = !!itemCont.querySelector(".saving");
        const hasDiscount =
          Number(sku.buyersRRP) > Number(sku.salePrice) &&
          !!sku.numberOfBottles;

        if (!hasExistingCustomPricing && hasVisibleSaving && hasDiscount) {
          let priceText =
            sku.numberOfBottles === 1
              ? "per bottle"
              : "per " + sku.numberOfBottles + " bottle case";
...
async function getPricingPanels() {
  ...
  for (const atbButton of allAtbButtons) {
    ...
    try {
      ...
      // Another run may have completed while we were waiting
      if (itemCont.querySelector(".offer-price-badge")) {
        itemCont.dataset.lt151PricingProcessed = "true";
        continue;
      }
      ...
      if (
        ...
        let dealTextEl = await nestedElementReady(itemCont, ".promo-rule");
        if (!dealTextEl) continue;

        // Guard again before mutating
        if (!dealTextEl.querySelector(".offer-price-badge")) {
        ...
        // move strike through price for single wine popups at the top of deal text
        let fullPriceStriked = clonedPopup.querySelector(
          '.straight-sku-wrapper [data-testid="strike-price"]',
        );
        let priceSection = clonedPopup.querySelector(
          '.straight-sku-wrapper [data-testid="price-section"]',
        );

        if (priceSection && fullPriceStriked) {
          priceSection.prepend(fullPriceStriked);
        }

        // add styling for some popups where pricing text is all in one element
        ...
        // create and add custom deal popup
        ...
        if (dealStuff) {
          ...
          let badge = itemCont.querySelector(".offer-price-badge");
          if (badge && !badge.querySelector(".deal-popup-container")) {
            badge.prepend(customDealPopup);
          }
        }
      }
...
function showPopup(badge) {
  ...
  if (popup) {
    ...
    //console.log("LT151 (Variation 1) | Opens tooltip")
    adobeDataLayer.push({
      event: "targetClickEvent",
      eventData: {
        click: {
  ...
  if (!main.classList.contains("test-lt146")) {
    main.addEventListener("click", async (e) => {
      if (e.target.closest(".ais-Hits-item a")) {
        //	console.log("LT151 (Variation 1) | Product click-through on PLP")
        adobeDataLayer.push({
          event: "targetClickEvent",
          eventData: {
            click: {
      ...
      if (e.target.closest("#qty-increase")) {
        ...
        //console.log("LT151 (Variation 1) | Uses increase quantity button")
        adobeDataLayer.push({
          event: "targetClickEvent",
          eventData: {
            click: {
      ...
      if (e.target.closest("#qty-decrease")) {
        ...
        //console.log("LT151 (Variation 1) | Uses decrease quantity button")
        adobeDataLayer.push({
          event: "targetClickEvent",
          eventData: {
            click: {
      ...
      if (e.target.closest("#custom-atb")) {
        ...
        // Capture card + promo flag early (before any async work)
        ...
        // console.log("LT151 (Variation 1) | ATB on main PLP")
        adobeDataLayer.push({
          event: "targetClickEvent",
          eventData: {
            click: {
        ...
        try {
          ...
          // Log after dataLayer push, only for promo-pricing cards
          if (hasPromoPricing) {
            adobeDataLayer.push({
              event: "targetClickEvent",
              eventData: {
